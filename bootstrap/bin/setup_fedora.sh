#!/bin/sh

# Fedora 4 container to setup
REPO=${FCREPO_JMS_BASEURL:-http://fcrepo:8080/fcrepo/rest}
USER=${USER:-fedoraAdmin}
PASSWD=${PASSWD:-moo}

function wait_until_up {
    CMD="curl -u ${USER}:${PASSWD} --write-out %{http_code} --silent -o /dev/stderr ${REPO}"
    echo "Waiting for response from Fedora via ${CMD}"
    RESULT=$(${CMD})
    until [ ${RESULT} -lt 400 ] && [ ${RESULT} -gt 199 ]
    do
        echo "Trying again, result was ${RESULT}"
        RESULT=$(${CMD})
        sleep 1
    done

    echo "OK, fedora is up: ${RESULT}"
}


function create_binary {
    file_path="$1"
    repo_path="$2"
    content_type=$(file -ib "$file_path")

    if [ "$repo_path" == "/" ]; then
	repo_path=""
    fi
	
    repo_path="$REPO/$repo_path"
	
    msg="Creating binary at $repo_path for $file_path"
    echo $msg
    
    curl -# -u ${USER}:${PASSWD} -X PUT --upload-file "$file_path" -H "Content-Type: $content_type" "$repo_path"

    if [ $? -ne 0 ]; then
	echo "Failed"
	exit
    fi
}

function create_object {
    repo_path="$1"

    if [ "$repo_path" == "/" ]; then
	repo_path=""
    fi
	
    repo_path="$REPO/$repo_path"
    
    msg="Creating object at $repo_path"
    echo $msg

    echo Executing curl -# -u ${USER}:${PASSWD} -X PUT -H "Content-Type: text/turtle" "$repo_path"
    curl -# -u ${USER}:${PASSWD} -X PUT -H "Content-Type: text/turtle" "$repo_path"

    if [ $? -ne 0 ]; then
	echo "Failed"
	exit
    fi
}

function delete_object {
    repo_path="$REPO/$1"

    msg="Deleting object at $repo_path"
    echo $msg

    curl -u ${USER}:${PASSWD} -X DELETE "$repo_path"
    curl -u ${USER}:${PASSWD} -X DELETE "$repo_path/fcr:tombstone"
}

wait_until_up
create_object "contributors"
create_object "deposits"
create_object "files"
create_object "funders"
create_object "grants"
create_object "journals"
create_object "policies"
create_object "publications"
create_object "publishers"
create_object "repositories"
create_object "repositoryCopies"
create_object "submissions"
create_object "users"

curl -v -i -# -u ${USER}:${PASSWD} -X PUT -H "Content-Type: text/turtle" --data-binary "@acl.ttl" ${REPO}/.acl
curl -v -i -# -u ${USER}:${PASSWD} -X PUT -H "Content-Type: text/turtle" --data-binary "@authz.ttl" ${REPO}/.acl/authz
curl -v -i -# -u ${USER}:${PASSWD} -X PATCH -H "Content-Type: application/sparql-update" -d "INSERT { <> <http://www.w3.org/ns/auth/acl#accessControl> </fcrepo/rest/.acl> } WHERE {}" ${REPO}/
curl -v -i -# -u ${USER}:${PASSWD} -X PUT -H "Content-Type: text/turtle" --data-binary "@nih.ttl" ${REPO}/repositories/nih
curl -v -i -# -u ${USER}:${PASSWD} -X PUT -H "Content-Type: text/turtle" --data-binary "@js.ttl" ${REPO}/repositories/js
